...
// 职责：读盘执行 app/bundle.cjs（rspack 构建产物），导出插件类。
//
// 实现说明：
...
//   （routes/*.js 壳同样经 loadBundle() 取最新导出，单一事实源在 src/ 与 dist/app）。
...
export function loadBundle() {
  const bundlePath = path.join(dir, "app", "bundle.cjs");
  const code = readFileSync(bundlePath, "utf8");
  const mod = { exports: {} };
  const bundleDir = path.join(dir, "app");
  return new Function(
    "exports",
    "module",
    "require",
