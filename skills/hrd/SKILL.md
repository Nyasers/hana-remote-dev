---
name: hrd
description: "Remote Development (HRD) 插件使用手册：SSH 远程连接、连接配置管理、远程文件与命令操作。触发词：hrd、ssh、远程、连接、remote、配置文件、密钥、认证。用户以 /hrd 开头或提及上述词时，按本手册处理连接与配置动作。"
---

# HRD — 远程操作手册

HRD 插件让 Hanako 能像在本地一样操作远程主机（文件、命令、交互会话）。用户说 `/hrd` 或提及远程操作时，按本手册执行。
